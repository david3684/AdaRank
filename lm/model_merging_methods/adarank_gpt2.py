...
class AdaRankModule(nn.Module):
    ...
    def forward(self, idx, input_id, attention_mask):
        ...
        self.base_model.config = AutoConfig.from_pretrained(
                    pretrained_model_name_or_path=os.path.join(WEIGHT_DIR, f"gpt2_{self.config.tasks[idx]}"))
        out = torch.nn.utils.stateless.functional_call(self.base_model, self._merged_state_dict, args=(), kwargs={'input_ids': input_id, 'attention_mask': attention_mask})
